(function ($) {
    ...
    $("form").on('submit', function () {
        ...
        form.find('input[data-required], textarea[data-required], select[data-required]').each(function () {
            ...
            if ($(this).is(':checkbox')) {
                // Проверка чек-боксов
                if (!$(this).is(':checked')) {
                    $(this).closest('.input-group').append(`<div class="error">${errorText}</div>`);
        ...
        form.find('input.inpt-email').each(function () {
            let emailInput = $(this),
                pattern = /^([a-z0-9_\.-])+@[a-z0-9-]+\.([a-z]{2,4}\.)?[a-z]{2,4}$/i;
            if (emailInput.val().trim() && !pattern.test(emailInput.val())) {
                emailInput.closest('.input-group').append(`<div class="error">Invalid email address</div>`);
    ...
    $('input, textarea, select').on('click focus change', function () {
        ...
        $(this).closest('.input-group').find('.error').remove();
    });
